...
-- Step 1: Create the backup user (handle existing user gracefully)
DO $$
BEGIN
    -- Check if user exists
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'supabase_backup_user') THEN
        RAISE NOTICE 'Creating new backup user';
        CREATE USER supabase_backup_user WITH PASSWORD 'backup_secure_password_2024';
    ELSE
        RAISE NOTICE 'Backup user already exists, updating password and permissions';
        -- Update password for existing user
        ALTER USER supabase_backup_user WITH PASSWORD 'backup_secure_password_2024';
    END IF;
END
$$;

-- Step 2: Drop existing policies first (if tables exist) to avoid conflicts
DO $$
BEGIN
...
-- Step 3: Grant necessary schema access
...
-- Step 4: Grant SELECT access to all tables in public schema
...
-- Step 5: Grant access to sequences (for complete backups)
...
-- Step 6: Create RLS policies for backup user to access all data
...
COMMENT ON ROLE supabase_backup_user IS 'Dedicated user for database backups with RLS policies allowing full read access - Graceful existing user handling'; 